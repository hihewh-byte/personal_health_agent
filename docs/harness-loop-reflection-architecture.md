...
### 6.1 核心差异化（Unique Value）

1. **Harness-first evolution**：不是「让 LLM 变聪明」，而是让 **证据冻结 + 后置审计** 不变的条件下，进化 **识别资产与用户事实**。  
...
## Appendix A — LinkedIn Introduction (≈200 words)

Most “self-improving agents” tune prompts or weights online—and quietly break production. We took a different path with **Personal Health Agent (PHA)** and **harness-core**: evolution is **offline, proposal-only, and human-gated**, while the control plane stays frozen.

Every turn still runs **Plan → Compose → Post-Audit → Done**. Evidence is locked before the LLM speaks; numerics and wearable comparisons are deterministic. What evolves is Layer 1 assets—intent catalogs, bilingual templates, schema triggers—and per-user **facts** compiled into a Chronic Health Brief with provenance, not guessed labels.

Our **Loop Engineering** pipeline turns stress-test JSONL and telemetry into a **failure taxonomy**, Reflection critique, and `pha.loop_proposal/v2` drafts. A **1E keyword veto** and nightly **148+164** regression batteries block bad merges—no auto-merge to main, no per-user routing forks.

The result: global recognition gets sharper for everyone; individual answers get more personal as labs, wearables, and chat notes accumulate—always with refs users (and auditors) can trace.

If you care about **trustworthy agent harnesses** rather than magic memory, this architecture is built for operators, not demos.
